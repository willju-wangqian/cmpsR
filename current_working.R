...
ccr <- get_ccf2(segments$segs[[nseg]], y, segments$index[[nseg]])
plot(ccr$lag, ccr$ccf, type = 'l')

# the result obtained by get_ccf3():
# get_ccf3() requires x to be the longer signature; other than that, 
# everything else is just the same as the  

ccr3 <- get_ccf3(y, segments$segs[[nseg]], min.overlap = length(segments$segs[[nseg]]))
plot(ccr3$lag, ccr3$ccf, type = 'l')


plot(ccr3$lag - segments$index[[nseg]][1], ccr3$ccf, type = 'l')
